...
/* FIXME:
  ...
  This was in the the downstream binutils-2.14-PS2.patch, but the upstream
  commit history suggests that binutils-2.15 deprecates both macros.
  The bundled libtool additionally fails to resolve elfcode.o symbols in
  libbfd.a while linking the final binutils products.

  As a kludge, the the macro expansion is pasted here to avoid fixing or
  upgrading the build harness, which is brittle.

  NB: include/elf/common.h

    #define ELF32_R_INFO(s,t)  (((s) << 8) + ((t) & 0xff))
    #define ELF64_R_INFO(s,t)  (((bfd_vma) (s) << 32) + (bfd_vma) (t))
*/
#if ARCH_SIZE == 32
			  irela->r_fino = ((0) << 8) + ((irela->r_info) & 0xff);
#endif
#if ARCH_SIZE == 64
			  irela->r_fino = ((bfd_vma) (0) << 32) + (bfd_vma) (irela->r_info);
#endif
			  *rel_hash = NULL;
			}
